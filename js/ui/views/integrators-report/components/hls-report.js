...
export const hlsReportTemplate = (viewModel) => {
    ...
    const networkContent = html`
        ...
        ${network.contentSteering ? statCardTemplate({
            label: 'Content Steering Server',
            value: network.contentSteering.serverUri,
            tooltip: 'The URI of the Content Steering manifest for server-side CDN redundancy and load balancing.',
            isoRef: 'HLS: 4.4.6.6',
        }) : ''}
    `;
    ...
    const timingContent = timing ? html`
        ...
        ${statCardTemplate({
            ...
            value: timing.dvrWindow ? `${timing.dvrWindow.toFixed(2)}s` : 'N/A',
        ...
        ${timing.lowLatency.active ? statCardTemplate({
            ...
            tooltip: 'The server-advertised target latency (PART-HOLD-BACK), providing a concrete goal for player tuning.',
            isoRef: 'HLS: 4.4.3.8',
        }) : ''}
        ${timing.lowLatency.active ? statCardTemplate({
            label: 'Part Target Duration',
            value: timing.partTargetDuration ? `${timing.partTargetDuration}s` : 'N/A',
            tooltip: 'The target duration for Low-Latency HLS Partial Segments.',
            isoRef: 'HLS: 4.4.3.7',
        }) : ''}
        ${timing.lowLatency.active ? statCardTemplate({
            ...
            isCode: false,
            tooltip: 'Indicates if the server supports blocking playlist reloads, allowing for more efficient updates.',
            isoRef: 'HLS: 4.4.3.8',
        }) : ''}
    ...
    const securityContent = html`
        ...
        ${listCardTemplate({
            ...
            tooltip: 'The Key IDs found in the manifest, crucial for debugging decryption. Note: KEYID is not part of the HLS RFC but is common practice.',
            isoRef: 'Vendor-Specific',
        })}
        ${statCardTemplate({
            label: 'HLS Encryption Method',
            value: security.hlsEncryptionMethod,
            tooltip: 'The specific encryption method used (e.g., SAMPLE-AES, AES-128).',
            isoRef: 'HLS: 4.3.2.4',
        })}
    `;
