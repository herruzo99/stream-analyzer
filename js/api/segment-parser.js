                ...
                (item) => html`
                    ...
                    <div
                        class="p-2 border-b border-r border-gray-700 font-mono ${item.isDifferent
                            ...
                            : ''}"
                    >
                        ${typeof item.val1 === 'object'
                            ? unsafeHTML(item.val1)
                            : item.val1}
                    </div>
                    <div
                        class="p-2 border-b border-gray-700 font-mono ${item.isDifferent
                            ? 'bg-red-900/50 text-red-300'
                            : ''}"
                    >
                        ${typeof item.val2 === 'object'
                            ? unsafeHTML(item.val2)
                            : item.val2}
                    </div>
                `
            )}
            ...
            case 'mvhd': {
                ...
                if (version === 1) {
                    box.details['creation_time'] = new Date(
                        Number(view.getBigUint64(12)) * 1000 - 2082844800000
                    ).toISOString();
                    box.details['modification_time'] = new Date(
                        Number(view.getBigUint64(20)) * 1000 - 2082844800000
                    ).toISOString();
                    box.details['timescale'] = view.getUint32(28);
                    box.details['duration'] = Number(view.getBigUint64(32));
                } else {
                    box.details['creation_time'] = new Date(
                        view.getUint32(12) * 1000 - 2082844800000
                    ).toISOString();
                    box.details['modification_time'] = new Date(
                        view.getUint32(16) * 1000 - 2082844800000
                    ).toISOString();
            ...
            case 'sidx': {
                ...
                if (ref_count > 0) {
                    ...
                    box.details['referenced_size_1'] =
                        view.getUint32(offset) & 0x7fffffff;
    ...
    try {
        if (mimeType === 'video/mp2t') {
            ...
            if (bufferB) {
                ...
                // For simplicity, we'll diff the top-level box details
                const diff = diffObjects(
                    boxesA[0]?.details || {},
                    boxesB[0]?.details || {}
                );
                ...
                render(isoAnalysisTemplate(boxesA), dom.modalContentArea);
            }
        }
    } catch (err) {
