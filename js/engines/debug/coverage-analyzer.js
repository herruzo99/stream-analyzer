 ...
 * @returns {import('../../core/types.js').CoverageFinding[]}
 */
    ...
    const walk = (node, schemaType, path) => {
        ...
        if (!schema) {
            findings.push({
                ...
                type: 'element',
                name: schemaType,
                details: 'This element type is not defined in the parser schema.',
            });
            return;
        }
...
const irSchema = {
    Representation: [
        'id', 'codecs', 'bandwidth', 'width', 'height', 'frameRate', 'sar',
        'mimeType', 'profiles', 'qualityRanking', 'selectionPriority', 'codingDependency',
        'scanType', 'dependencyId', 'associationId', 'associationType', 'segmentProfiles',
        'mediaStreamStructureId', 'maximumSAPPeriod', 'startWithSAP', 'maxPlayoutRate',
        'tag', 'eptDelta', 'pdDelta', 'representationIndex', 'failoverContent',
        'audioChannelConfigurations', 'framePackings', 'ratings', 'viewpoints',
        'accessibility', 'labels', 'groupLabels', 'subRepresentations', 'resyncs',
        'outputProtection', 'extendedBandwidth', 'videoRange', 'stableVariantId',
        'pathwayId', 'supplementalCodecs', 'reqVideoLayout', 'serializedManifest'
    ],
    AdaptationSet: [
        'id', 'contentType', 'lang', 'mimeType', 'profiles', 'group',
        'bitstreamSwitching', 'maxWidth', 'maxHeight', 'maxFrameRate',
        'representations', 'contentProtection', 'framePackings', 'ratings',
        'viewpoints', 'accessibility', 'labels', 'groupLabels', 'roles',
        'contentComponents', 'resyncs', 'outputProtection', 'stableRenditionId',
        'bitDepth', 'sampleRate', 'channels', 'assocLanguage',
        'characteristics', 'forced', 'serializedManifest'
    ],
    Period: [
        'id', 'start', 'duration', 'bitstreamSwitching', 'assetIdentifier',
        'adaptationSets', 'subsets', 'preselections', 'serviceDescriptions',
        'eventStreams', 'events', 'serializedManifest'
    ],
    Manifest: [
        'id', 'type', 'profiles', 'minBufferTime', 'publishTime', 'availabilityStartTime',
        'timeShiftBufferDepth', 'minimumUpdatePeriod', 'duration', 'maxSegmentDuration',
        'maxSubsegmentDuration', 'programInformations', 'metrics', 'locations',
        'patchLocations', 'serviceDescriptions', 'initializationSets', 'segmentFormat',
        'events', 'periods', 'serializedManifest', 'summary', 'serverControl',
        'hlsDefinedVariables', 'tags', 'isMaster', 'variants', 'segments',
        'preloadHints', 'renditionReports', 'partInf'
    ]
};
 ...
 * @param {import('../../core/types.js').Manifest} manifestIR The parsed manifest IR.
 * @returns {import('../../core/types.js').CoverageFinding[]}
 */
