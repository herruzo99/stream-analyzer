...
`;
  ...
  var getTagHTML = (tagName) => {
    ...
    return o2(`&lt;${isClosing ? "/" : ""}<span class="${tagClass}" ${tooltipAttrs}>${cleanTagName}</span>`);
  };
  var getAttributeHTML = (tagName, attr) => {
    ...
    return o2(`<span class="${nameClass}" ${tooltipAttrs}>${attr.name}</span>=<span class="${valueClass}">"${escapeHtml(attr.value)}"</span>`);
  };
    ...
    const preformatted = (node, depth = 0) => {
      ...
      switch (node.nodeType) {
        case Node.ELEMENT_NODE: {
          ...
          const meaningfulChildren = childNodes.filter((n2) => n2.nodeType === Node.ELEMENT_NODE || n2.nodeType === Node.TEXT_NODE && n2.textContent.trim());
        ...
        case Node.TEXT_NODE: {
          return node.textContent.trim() ? x`${indent}<span class="interactive-xml-text">${escapeHtml(node.textContent.trim())}</span>\n` : E;
        }
  ...
  var isoBoxTemplate = (box) => {
    ...
    const detailsTemplate = Object.keys(box.details).length > 0 ? x`
    ...
    const childrenTemplate = box.children.length > 0 ? x`<ul>${box.children.map((child) => x`<li>${isoBoxTemplate(child)}</li>`)}</ul>` : "";
    ...
    try {
      ...
      switch (box.type) {
        ...
        case "mvhd": {
          ...
          if (version === 1) {
            box.details["creation_time"] = new Date(Number(view.getBigUint64(12)) * 1e3 - 20828448e5).toISOString();
            box.details["modification_time"] = new Date(Number(view.getBigUint64(20)) * 1e3 - 20828448e5).toISOString();
            box.details["timescale"] = view.getUint32(28);
            box.details["duration"] = Number(view.getBigUint64(32));
          } else {
            box.details["creation_time"] = new Date(view.getUint32(12) * 1e3 - 20828448e5).toISOString();
            box.details["modification_time"] = new Date(view.getUint32(16) * 1e3 - 20828448e5).toISOString();
        ...
        case "mdhd": {
          ...
          box.details["language"] = String.fromCharCode((lang >> 10 & 31) + 96, (lang >> 5 & 31) + 96, (lang & 31) + 96);
          break;
        }
        case "hdlr": {
        ...
        case "sidx": {
          ...
          let offset = 20;
          if (version === 1) {
            box.details["earliest_presentation_time"] = Number(view.getBigUint64(offset));
            offset += 8;
            box.details["first_offset"] = Number(view.getBigUint64(offset));
            offset += 8;
          } else {
        ...
        case "tfhd": {
          ...
          let offset = 16;
          if (flags & 1) {
            box.details["base_data_offset"] = Number(view.getBigUint64(offset));
            offset += 8;
          }
          if (flags & 2) {
        ...
        case "trun": {
          ...
          box.details["samples"] = x`<div class="sample-data"><pre>${samples.join("\n")}${sample_count > 10 ? `
... (${sample_count - 10} more)` : ""}</pre></div>`;
  ...
  function dispatchAndRenderSegmentAnalysis(e4, buffer) {
    if (!buffer) {
      B(x`<p class="fail">Segment buffer is not available for analysis.</p>`, dom.modalContentArea);
      return;
    }
    const target = (
      /** @type {HTMLElement} */
    ...
    try {
      ...
      B(x`<p class="fail">Could not parse segment buffer: ${err.message}.</p>`, dom.modalContentArea);
    }
  }
  ...
  async function loadAndRenderSegmentRange(mode) {
    ...
    B(
      x`<p class="info">Fetching segment data...</p>`,
      contentArea
    );
  ...
  async function handleFilter() {
    ...
    if (isNaN(fromValue) || isNaN(toValue)) {
      B(x`<p class="warn">Please enter valid numbers for the filter range.</p>`, contentArea);
      return;
    }
    B(x`<p class="info">Filtering and fetching segments...</p>`, contentArea);
    ...
    if (segmentsToFetch.length === 0) {
      B(x`<p class="warn">No segments found matching the specified filter.</p>`, contentArea);
      return;
    }
    await Promise.all(
      segmentsToFetch.map((seg) => fetchSegment(seg.resolvedUrl))
...
`;
  ...
  var mpdUpdatesTemplate = (stream) => {
    ...
    if (updateCount === 0) {
      ...
      const formattedInitialMpd = (0, import_xml_formatter2.default)(initialMpdString, {
        indentation: "  ",
        lineSeparator: "\n"
      });
  ...
  var exampleStreams = [
    ...
    {
      name: "DASH-IF: Transport Stream (On-Demand)",
      url: "https://dash.akamaized.net/akamai/test/isoff-on-demand-ts/test.mpd"
    },
    {
