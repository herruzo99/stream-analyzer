...
*   **Avoid Overwriting**: Do NOT use `write_to_file` with `Overwrite: true` on existing files unless you are rewriting the *entire* file from scratch (which should be rare for large files).
*   **File Operations**: Use `run_command` for `mv`, `rm`, `mkdir`, `touch`.

### 4. Test-First Implementation
*   **Bug Fixes**: Generate a failing test case that reproduces the bug *before* fixing it.
*   **New Features**: Generate a test that defines the API and expected behavior *before* implementing it.
