...
const updateItemTemplate = (update, isActive, onSelect) => {
    const { sequenceNumber, timestamp, changes, meta } = update;

    const activeClasses = 'border-blue-500 bg-blue-900/5';
    const inactiveClasses = 'border-slate-700/50';

    const containerClass = `relative pl-5 py-3 pr-3 border-l-[3px] cursor-pointer transition-all hover:bg-slate-800/40 group ${isActive ? activeClasses : inactiveClasses}`;

    const markerClass = `absolute left-[-5px] top-4 w-2 h-2 rounded-full transition-all ${isActive ? 'bg-blue-400 ring-4 ring-blue-500/20' : 'bg-slate-600 group-hover:bg-slate-500'}`;

    const headerTextClass = `font-mono text-sm font-bold ${isActive ? 'text-blue-400' : 'text-slate-200'}`;
    ...
    return html`
        <div class="${containerClass}" @click=${() => onSelect(update.id)}>
            <!-- Timeline Marker -->
            ...
            <div class="flex justify-between items-baseline mb-1.5">
                <div class="flex items-center gap-2">
                    <span class="${headerTextClass}">#${sequenceNumber}</span>
                    ${meta.label
            ...
            <!-- Changes Summary -->
            <!-- prettier-ignore -->
            <div class="flex flex-wrap gap-x-3 gap-y-1 text-xs font-medium opacity-80">
                ${changes.additions > 0
...
export const updateFeedTemplate = (
    updates,
    activeUpdateId,
    streamId,
    onSelect
    ...
    return html`
        ...
        >
            ...
            <div class="grow overflow-y-auto custom-scrollbar relative">
                ${updates.map((u) =>
                    updateItemTemplate(u, u.id === activeUpdateId, onSelect)
                )}
                ${updates.length === 0
                    ? html`
                          <div
