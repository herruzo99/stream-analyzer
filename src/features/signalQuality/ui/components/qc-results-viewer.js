...
const RADAR_METRICS = [
    { key: 'luma', label: 'Luminance', unit: 'Y', desc: 'Average brightness. Low values (<16) indicate black frames.' },
    { key: 'sharpness', label: 'Sharpness', unit: 'SI', desc: 'Edge frequency. Low values indicate blur or freeze.' },
    { key: 'audioLevel', label: 'Audio Lvl', unit: 'dB', desc: 'Normalized RMS. Low values (<-60dB) indicate silence.' },
    { key: 'contrastStdDev', label: 'Contrast', unit: 'σ', desc: 'Luma variance. Low values indicate washed-out video.' },
    { key: 'blockiness', label: 'Cleanliness', unit: 'Q', desc: 'Inverse blockiness score. Higher is cleaner.' },
    { key: 'predictedQuality', label: 'Quality', unit: '/100', desc: 'Predicted perceptual quality score (0-100).' },
    { key: 'banding', label: 'Banding', unit: 'Scr', desc: 'Banding artifact score. Lower is better.' }
];
...
const safeBtoa = (str) => btoa(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g, (match, p1) => String.fromCharCode(parseInt(p1, 16))));
...
const QC_TOOLTIP_DATA = {
    metrics: {
        ...
        'Banding': {
            ...
            technical: 'Gradient analysis detecting flat runs followed by small steps.'
        },
        'Quality': {
    ...
    issues: {
        black: {
            ...
            suggestion: 'Verify if this is a creative intent or a technical fault.'
        },
        freeze: {
        ...
        blocky: {
            ...
            suggestion: 'Increase bitrate or check encoder complexity settings.'
        },
        flat: {
...
const createTooltipContent = (label, metricData, description, unit = '') => {
    const def = QC_TOOLTIP_DATA.metrics[label] || { title: label, what: description, limits: 'N/A', impact: 'N/A' };

    // Handle both old scalar values and new object structure
    const isObject = metricData && typeof metricData === 'object';
    const value = isObject ? (metricData.avg?.toFixed(1) || 'N/A') : (metricData?.toFixed(1) || 'N/A');
...
const createIssueTooltip = (issue) => {
    const def = QC_TOOLTIP_DATA.issues[issue.type] || { title: issue.type, what: 'Unknown anomaly.', trigger: 'N/A', impact: 'N/A', suggestion: 'Investigate manually.' };
...
class QcResultsViewer extends HTMLElement {
    ...
    updateChart() {
        this.chartContainer = /** @type {HTMLElement} */ (this.querySelector('#qc-radar-chart'));
    ...
    handleTimelineMouseMove(e, duration) {
        ...
        const cursor = /** @type {HTMLElement} */ (this.querySelector('#timeline-cursor'));
        const timeLabel = /** @type {HTMLElement} */ (this.querySelector('#timeline-time-label'));
        if (cursor && timeLabel) {
    ...
    handlePlayIssue(e, issue) {
        ...
        uiActions.requestPlayerPlayback({ startTime: issue.startTime, autoPlay: true });
    }

    renderTimelineTracks(issues, duration, isLive, mediaStartTime = 0) {
        const { highlightedTimeRange, highlightedIssueId } = useUiStore.getState();
        ...
        if (sortedTypes.length === 0) {
            return html`
                <div class="h-24 flex items-center justify-center text-slate-600 text-xs italic">
        ...
        return html`
            <div class="flex flex-col relative w-full select-none"
                 @mousemove=${(e) => this.handleTimelineMouseMove(e, duration)}
                 @mouseleave=${() => this.handleTimelineMouseLeave()}>
                
                <!-- Time Cursor -->
                <div id="timeline-cursor" class="absolute top-0 bottom-0 w-px bg-white z-40 pointer-events-none hidden shadow-[0_0_10px_white]">
                     <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 bg-white text-black text-[9px] font-bold px-1.5 py-0.5 rounded font-mono shadow-sm" id="timeline-time-label">0.00s</div>
                </div>
            ...
            const colorClass = ISSUE_COLORS[type] || ISSUE_COLORS.default;

            return html`
                        <div class="flex items-stretch border-b border-slate-800/50 hover:bg-white/[0.01] transition-colors group/track min-h-[36px]">
                            <div class="w-24 shrink-0 flex items-center px-4 border-r border-slate-800/50">
                                <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 group-hover/track:text-slate-200 transition-colors">
                                    ${type}
                                </span>
                            </div>
                ...
                const relativeStart = issue.startTime - mediaStartTime;
                const effectiveStart = Math.min(relativeStart, duration);
                const effectiveEnd = Math.min(relativeStart + issue.duration, duration);

                let leftPct = (effectiveStart / duration) * 100;
                let widthPct = ((effectiveEnd - effectiveStart) / duration) * 100;
                ...
                let isHighlighted = highlightedIssueId === issue.id;
                if (!isHighlighted && highlightedTimeRange && !highlightedIssueId) {
                    const { start, end } = highlightedTimeRange;
                    const iStart = issue.startTime;
                    const iEnd = issue.startTime + issue.duration;
                    if (iEnd >= start && iStart <= end) isHighlighted = true;
                }
                ...
                const tooltipB64 = createIssueTooltip(issue);

                // Disable click if live
                const clickHandler = isLive ? null : (e) => this.handlePlayIssue(e, issue);
                ...
                return html`
                        ...
                        this.handleMouseEnterMetric(issue.type);
                        this.handleMouseEnterTime(issue.startTime, issue.startTime + issue.duration);
                    }}
                                            @mouseleave=${() => {
                ...
                <!-- Ruler -->
                <div class="flex h-6 border-t border-slate-800 text-[9px] text-slate-500 font-mono relative mt-1">
                    <div class="w-24 shrink-0 border-r border-slate-800 bg-slate-900/50"></div>
                    <div class="grow relative">
                        <span class="absolute left-1 top-1">${mediaStartTime.toFixed(0)}s</span>
                        <span class="absolute left-1/4 top-1 border-l border-slate-800 h-2 pl-1">${(mediaStartTime + duration * 0.25).toFixed(0)}s</span>
                        <span class="absolute left-1/2 top-1 border-l border-slate-800 h-2 pl-1">${(mediaStartTime + duration * 0.5).toFixed(0)}s</span>
                        <span class="absolute left-3/4 top-1 border-l border-slate-800 h-2 pl-1">${(mediaStartTime + duration * 0.75).toFixed(0)}s</span>
                        <span class="absolute right-1 top-1">${(mediaStartTime + duration).toFixed(1)}s</span>
                    </div>
                </div>
            </div>
        `;
    ...
    renderStatCard(label, metricData, unit, icon, color = 'text-white', isInverse = false) {
        ...
        const displayValue = isNA ? 'N/A' : (typeof rawValue === 'number' ? rawValue.toFixed(1) : rawValue);
        const displayColor = isNA ? 'text-slate-500' : color;
        const opacityClass = isNA ? 'opacity-60 grayscale' : 'opacity-100';

        const metricDef = RADAR_METRICS.find(m => m.label === label) || { desc: 'Performance metric', unit };
        const unitLabel = unit || metricDef.unit;

        // Pass full metricData to tooltip creator
        const tooltipB64 = createTooltipContent(label, metricData, metricDef.desc, unitLabel);
        ...
        return html`
            ...
            >
                <div>
                    <div class="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">${label}</div>
                    <div class="text-2xl font-mono font-black ${displayColor} tracking-tight">
                        ${displayValue}<span class="text-sm text-slate-600 ml-1 font-bold">${unitLabel}</span>
                    </div>
                </div>
    ...
    downloadCSV() {
        ...
        const headers = ['Frame', 'Segment', 'Relative Time (s)', 'Absolute Time (s)', 'Luma', 'Sharpness', 'Audio Level (dB)', 'Audio Peak (dB)', 'Blockiness', 'Contrast', 'Quality', 'Banding'];
        ...
        link.setAttribute('download', `qc_analysis_${this._data.streamId}_${new Date().toISOString()}.csv`);
    ...
    downloadIssuesCSV() {
        ...
        const headers = ['ID', 'Type', 'Relative Start (s)', 'Duration (s)', 'Value', 'Severity'];
        ...
        const link = document.createElement('a');
        link.setAttribute('href', url);
        link.setAttribute('download', `qc_issues_${this._data.streamId}_${new Date().toISOString()}.csv`);
        link.style.visibility = 'hidden';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    ...
    render() {
        ...
        if (trackDetails?.video && stream) {
            const t = stream.manifest?.summary?.videoTracks.find(vt => vt.id === trackDetails.video);
        ...
        if (trackDetails?.audio && stream) {
            const t = stream.manifest?.summary?.audioTracks.find(at => at.id === trackDetails.audio);
            if (t) audioLabel = t.label || t.lang?.toUpperCase() || trackDetails.audio;
        }

        const videoBadge = videoLabel ? html`
            <span class="text-[9px] px-1.5 py-0.5 rounded bg-blue-900/20 text-blue-300 border border-blue-500/20 font-bold flex items-center gap-1">
                ${icons.clapperboard} ${videoLabel}
            </span>
        ` : '';
        const audioBadge = audioLabel ? html`
            <span class="text-[9px] px-1.5 py-0.5 rounded bg-purple-900/20 text-purple-300 border border-purple-500/20 font-bold uppercase flex items-center gap-1">
                ${icons.audioLines} ${audioLabel}
            </span>
        ` : '';

        const { highlightedQcMetric, highlightedTimeRange, highlightedIssueId } = useUiStore.getState();
        ...
        const targetTypes = !isIdHoverActive && !isTimeHoverActive && highlightedQcMetric
            ? (METRIC_TO_ISSUE_MAP[highlightedQcMetric] || [highlightedQcMetric])
            : [];

        const template = html`
            <div class="h-full flex flex-col bg-slate-950 text-slate-200 overflow-hidden">
                <!-- Header -->
                <div class="shrink-0 p-6 border-b border-slate-800 bg-slate-900/50 flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-xs text-slate-400 font-mono mt-1">Status: ${selectedJob?.status || 'Complete'}</p>
                    </div>
                    <div class="flex items-center gap-3">
                        <button @click=${() => this.downloadCSV()} class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg transition-all flex items-center gap-2 shadow-lg shadow-blue-900/20">
                            ${icons.download} Export Metrics
                        </button>
                        <button @click=${() => this.downloadIssuesCSV()} class="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-xs font-bold rounded-lg transition-all flex items-center gap-2 shadow-lg shadow-red-900/20">
                            ${icons.alertTriangle} Export Issues
                        </button>
                        <button @click=${onReset} class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold rounded-lg border border-slate-700 transition-all flex items-center gap-2">
                            ${icons.arrowLeft} Back to Grid
                        </button>
                    </div>
                </div>

                <div class="grow overflow-y-auto custom-scrollbar p-6 space-y-6">
                    
                    <!-- Top Stats Grid -->
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[280px]">
                        <!-- ECharts Radar & Score -->
                        <div class="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 flex relative overflow-hidden group">
                             <div class="absolute top-4 left-4 z-10 pointer-events-none">
                                 <div class="text-xs font-bold text-slate-500 uppercase tracking-wider">Quality Score</div>
                                 <div class="text-5xl font-black ${scoreColor} mt-1 drop-shadow-md">${score}</div>
                             </div>
                             <div id="qc-radar-chart" class="w-full h-full"></div>
                        </div>

                        <!-- Key Metrics -->
                        <div class="lg:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
                            ${this.renderStatCard('Avg Luma', metrics.luma, 'Y', icons.sun, 'text-blue-400')}
                            ${this.renderStatCard('Audio Peak', metrics.peakAudioLevel, 'dB', icons.volumeUp, metrics.peakAudioLevel?.avg > -1 ? 'text-red-400' : 'text-pink-400')}
                            ${this.renderStatCard('Sharpness', metrics.sharpness, '', icons.camera)}
                            ${this.renderStatCard('Cleanliness', metrics.blockiness, '%', icons.grid, metrics.blockiness?.avg > 20 ? 'text-amber-400' : 'text-slate-300', true)}
                            ${this.renderStatCard('Contrast', metrics.contrastStdDev, 'SD', icons.activity)}
                            ${this.renderStatCard('Banding', metrics.banding, 'Scr', icons.layers, metrics.banding?.avg > 15 ? 'text-pink-400' : 'text-slate-300')}
                            ${this.renderStatCard('Quality', metrics.predictedQuality, '/100', icons.star, score > 80 ? 'text-emerald-400' : 'text-amber-400')}
                            ${this.renderStatCard('Issues', issues.length, '', icons.alertTriangle, issues.length > 0 ? 'text-red-400' : 'text-emerald-400')}
                        </div>
                    </div>

                    <!-- Swimlane Timeline -->
                    <div class="bg-slate-900/30 border border-slate-800 rounded-xl overflow-hidden">
                        <div class="px-4 py-3 bg-slate-900/80 border-b border-slate-800 flex justify-between items-center">
                            ...
                            <span class="text-[10px] text-slate-500 font-mono">0s - ${scanDuration}s</span>
                        </div>
                        <div class="bg-slate-950/50">
                            ${this.renderTimelineTracks(issues, scanDuration, isLive, selectedJob?.mediaStartTime || 0)}
                        </div>
                    </div>

                    <!-- Issues Table -->
                    <div class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden flex flex-col min-h-[300px]">
                        <div class="grid grid-cols-[80px_120px_1fr_100px_80px] gap-4 px-6 py-3 bg-slate-950 border-b border-slate-800 text-[10px] font-bold text-slate-500 uppercase tracking-wider sticky top-0 z-10">
                            <div>Time</div>
                            <div>Type</div>
                        ...
                        <div class="divide-y divide-slate-800/50 bg-slate-900/50">
                            ${issues.length === 0
                ? html`<div class="p-12 text-center text-slate-500 italic">No anomalies detected. Stream is clean.</div>`
                : issues.map((issue, idx) => {
                    const tooltipB64 = createIssueTooltip(issue);

                    let isHighlighted = false;
                    if (isIdHoverActive) {
                        if (issue.id === highlightedIssueId) isHighlighted = true;
                        ...
                        if (targetTypes.includes(issue.type)) isHighlighted = true;
                        if (highlightedQcMetric === issue.type) isHighlighted = true;
                    }
                    ...
                    const onRowEnter = () => {
                        ...
                        for (const [key, types] of Object.entries(METRIC_TO_ISSUE_MAP)) {
                            if (types.includes(issue.type)) {
                                metricKey = key;
                                break;
                            }
                        }
                        this.handleMouseEnterMetric(metricKey);
                        this.handleMouseEnterTime(issue.startTime, issue.startTime + issue.duration);
                    };
                    ...
                    return html`
                                        <div 
                                            ...
                                            data-tooltip-html-b64="${tooltipB64}"
                                        >
                                            <div class="font-mono text-slate-400">${issue.startTime.toFixed(2)}s</div>
                                            <div>
                                                <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase border bg-slate-800 border-slate-700 text-red-300">
                                                    ${issue.type}
                                                </span>
                                            </div>
                                            <div class="font-mono text-slate-300 truncate pr-4">${issue.value}</div>
                                            <div class="text-right font-mono text-slate-500">${issue.duration.toFixed(3)}s</div>
                                            <div class="text-right">
                                                ${isLive
                            ? html`
                                                        <span class="text-slate-600 text-[10px] cursor-not-allowed ${tooltipTriggerClasses}" 
                                                              data-tooltip="Replay disabled for Live stream">
                                                            ${icons.play}
                                                        </span>`
                            : html`
                                                        <button 
                                                            @click=${(e) => this.handlePlayIssue(e, issue)}
...
export const qcResultsTemplate = (data) => html`<qc-results-viewer .data=${data}></qc-results-viewer>`;