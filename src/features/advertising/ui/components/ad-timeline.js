...
export const adTimelineTemplate = (avails, duration, onSelect, selectedId) => {
        ...
        return html`
            ...
            >
                <p class="text-xs text-slate-500 uppercase tracking-wider">
                    Live Stream - Relative Timeline Unavailable
                </p>
            </div>
        `;
    }
    ...
    return html`
        <div
            class="relative w-full h-12 bg-slate-900/50 rounded-lg border border-slate-800 mb-6 select-none overflow-hidden"
        >
            <!-- Base Track -->
            <div
            ...
            ${avails.map((avail) => {
                ...
                const color = avail.statusColor; // 'emerald', 'amber', 'blue'

                // Tailwind mapping for dynamic colors
                const bgClass = {
                ...
                return html`
                    <div
                        ...
                        class="absolute top-1/2 -translate-y-1/2 h-6 min-w-[8px] rounded-sm cursor-pointer transition-all duration-200 z-10 ${bgClass} ${shadowClass} ${tooltipTriggerClasses}"
                        style="left: ${avail.timelineStyles
                            .left}; width: ${avail.timelineStyles.width};"
