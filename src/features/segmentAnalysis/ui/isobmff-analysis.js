 ...
 * @param {string} key The key/header of the value.
 * @returns {string} The formatted value.
 */
const formatSampleValue = (value, key) => {
    if (value === undefined || value === null) {
        return 'N/A';
    }
    if (key === 'iv' && value instanceof Uint8Array) {
        return Array.from(value)
            .map((b) => b.toString(16).padStart(2, '0'))
            .join('');
    }
    if (key === 'subsamples' && Array.isArray(value)) {
