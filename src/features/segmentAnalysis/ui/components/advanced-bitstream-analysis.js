...
class AdvancedBitstreamAnalysis extends HTMLElement {
    ...
    computeDistribution(frames) {
        if (!frames.length) return { categories: [], dataI: [], dataP: [], dataB: [] };

        const maxSize = Math.max(...frames.map(f => f.size));
        const bucketCount = 20;
        const bucketSize = Math.ceil(maxSize / bucketCount);

        const buckets = new Array(bucketCount).fill(0).map(() => ({ I: 0, P: 0, B: 0 }));
        ...
        frames.forEach(f => {
            const bucketIdx = Math.min(Math.floor(f.size / bucketSize), bucketCount - 1);
            let type = 'B';
    ...
    updateCharts() {
        ...
        if (mainContainer) {
            ...
            try { this.resizeObserver.observe(mainContainer); } catch (_e) { /* ignore */ }
        }
        ...
        if (distContainer) {
            ...
            try { this.resizeObserver.observe(distContainer); } catch (_e) { /* ignore */ }
        }
        ...
        if (gopContainer) {
            ...
            if (gopData.length > 0) {
                ...
                try { this.resizeObserver.observe(gopContainer); } catch (_e) { /* ignore */ }
            }
        }
    }
    ...
    renderMainChart(container, frames) {
        ...
        const mapFrameData = (typeCheck) => {
            return frames.map((f, i) => {
                ...
                return {
                    value: f.size,
                    meta: { type: f.type, nals: f.nalTypes ? f.nalTypes.join(',') : '' }
                };
            });
        };

        const dataI = mapFrameData(f => f.isKeyFrame);
        const dataP = mapFrameData(f => f.type && f.type.includes('P') && !f.isKeyFrame);
        const dataB = mapFrameData(f => !f.isKeyFrame && !f.type.includes('P'));
        ...
        const options = {
            ...
            tooltip: {
                ...
                formatter: (params) => {
                    const item = params.find(p => p.value !== undefined && p.value !== null);
                    if (!item) return '';
            ...
            dataZoom: [
                ...
                { type: 'slider', bottom: 0, height: 16, borderColor: 'transparent', backgroundColor: '#1e293b' }
            ],
            xAxis: {
                type: 'category',
                data: frameIndices,
            ...
            yAxis: {
                ...
                axisLabel: { color: '#64748b', fontSize: 10, formatter: (val) => `${(val / 1024).toFixed(0)}k` },
    ...
    renderDistChart(container, { categories, dataI, dataP, dataB }) {
        const options = {
            ...
            series: [
                { name: 'I', type: 'bar', stack: 'total', data: dataI, itemStyle: { color: '#ef4444' } },
                { name: 'P', type: 'bar', stack: 'total', data: dataP, itemStyle: { color: '#3b82f6' } },
                { name: 'B', type: 'bar', stack: 'total', data: dataB, itemStyle: { color: '#6366f1' } }
            ]
        };
        renderChart(container, options);
    }
            ...
            yAxis: [
                {
                    ...
                    name: 'Size',
                    position: 'left',
                    nameTextStyle: { color: '#10b981', fontSize: 9, padding: [0, 0, 0, 10] },
                    axisLabel: {
                        color: '#10b981',
                        fontSize: 9,
    ...
    render() {
        ...
        const metricCard = (title, value, subtext, color = 'text-white', tooltipB64) => html`
            <div
            ...
            >
                <span class="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1 flex justify-between">
                    ${title}
                    <span class="text-slate-600 group-hover:text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity">${icons.info}</span>
                </span>
                <div class="flex items-baseline gap-2">
                    <span class="text-xl font-mono font-black ${color}">${value}</span>
                    ${subtext ? html`<span class="text-[9px] text-slate-400 font-medium truncate">${subtext}</span>` : ''}
                </div>
            </div>
        `;

        const bppTooltip = this.createTooltip('Bits Per Pixel (BPP)', 'A quality heuristic. Formula: Bitrate / (Width × Height × FPS). Higher is usually better.');
        const effTooltip = this.createTooltip('I/P Ratio', 'Ratio of average I-frame size to P-frame size. Higher values indicate more efficient inter-frame compression.');
        const varTooltip = this.createTooltip('Variability', 'Bitrate variance (Standard Deviation / Mean). High variability indicates aggressive VBR.');
        const gopTooltip = this.createTooltip('GOP Structure', `Group of Pictures length. Detected structure: ${gopStructure}`);

        const qualityVal = (typeof bpp === 'number' && !isNaN(bpp)) ? bpp.toFixed(3) : 'N/A';
        const efficiencyVal = (typeof iFrameRatio === 'number' && !isNaN(iFrameRatio)) ? iFrameRatio.toFixed(1) + 'x' : 'N/A';
        const stabilityVal = (typeof variability === 'number' && !isNaN(variability)) ? (variability * 100).toFixed(0) + '%' : 'N/A';
        const gopVal = (typeof gopLength === 'number' && !isNaN(gopLength)) ? gopLength.toFixed(1) : 'N/A';
        ...
        const template = html`
            <div class="flex flex-col gap-4 h-full">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 shrink-0">
                    ${metricCard('Quality', qualityVal, 'BPP', (bpp < 0.1 && bpp > 0) ? 'text-yellow-400' : 'text-emerald-400', bppTooltip)}
                    ${metricCard('Efficiency', efficiencyVal, 'I/P Ratio', 'text-blue-400', effTooltip)}
                    ${metricCard('Stability', stabilityVal, 'Variance', variability > 0.5 ? 'text-orange-400' : 'text-white', varTooltip)}
                    ${metricCard('GOP', gopVal, gopStructVal, 'text-purple-400', gopTooltip)}
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 grow min-h-0">
                    ...
                    <div class="lg:col-span-2 bg-slate-900 rounded-xl border border-slate-800 p-1 flex flex-col shadow-sm relative overflow-hidden">
                    ...
                    <div class="flex flex-col gap-4 min-h-0">
                        <!-- Distribution -->
                        <div class="flex-1 bg-slate-900 rounded-xl border border-slate-800 p-1 flex flex-col relative overflow-hidden">
                             <div class="absolute top-3 left-4 z-10 pointer-events-none">
                                 <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider">Size Distribution</h3>
                             </div>
                             <div class="grow relative">
                                 <div id="chart-dist" class="absolute inset-0 w-full h-full"></div>
                             </div>
                        </div>
                        ...
                        <div class="flex-1 bg-slate-900 rounded-xl border border-slate-800 p-1 flex flex-col relative overflow-hidden">
                             <div class="absolute top-3 left-4 z-10 pointer-events-none">
                                 <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider">GOP Consistency</h3>
                             </div>
                             <div class="grow relative">
                                 <div id="chart-gop" class="absolute inset-0 w-full h-full"></div>
                             </div>
                        </div>
                    </div>
...
export const advancedBitstreamAnalysisTemplate = (bitstreamData) =>
    html`<advanced-bitstream-analysis .data=${bitstreamData}></advanced-bitstream-analysis>`;