...
const controlSectionTemplate = (title, content, disabled = false) => html`
    <div
        class="bg-gray-800 p-4 rounded-lg transition-opacity h-full ${disabled
            ...
            : ''}"
    >
        <h4 class="font-bold text-gray-300 mb-3 text-sm uppercase tracking-wider">${title}</h4>
        <div ?disabled=${disabled}>${content}</div>
    </div>
`;

const dropdownButton = (label, subtext, onClick, isActive = false) => html`
    <button @click=${onClick} class="input-field text-left flex items-center justify-between w-full hover:bg-gray-600 transition-colors ${isActive ? 'ring-1 ring-blue-500 border-blue-500' : ''}">
        <span class="truncate grid grid-cols-1">
            <span class="font-semibold text-gray-200 truncate">${label}</span>
            ${subtext ? html`<span class="text-xs text-gray-400 truncate">${subtext}</span>` : ''}
        </span>
...
const modeToggleTemplate = (currentMode) => html`
    <div class="flex bg-gray-900/50 p-1 rounded-lg border border-gray-700/50 w-fit">
        <button
            @click=${() => uiActions.setPlayerControlMode('standard')}
            class="px-3 py-1.5 text-xs font-bold rounded-md transition-colors ${currentMode === 'standard' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-400 hover:text-gray-200'}"
        >
            Standard
        </button>
        <button
            @click=${() => uiActions.setPlayerControlMode('advanced')}
            class="px-3 py-1.5 text-xs font-bold rounded-md transition-colors ${currentMode === 'advanced' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-400 hover:text-gray-200'}"
        >
            Advanced
        </button>
    </div>
...
const findActivePreset = (presets, currentConfigSubset) => {
    return presets.find(preset => {
        return Object.entries(preset.config).every(([key, value]) => currentConfigSubset[key] === value);
    });
};
        ...
        <div class="grid grid-cols-1 gap-4">
            <div>
                ...
                ${dropdownButton(
                    ...
                    activeAbrPreset.description || 'Custom configuration applied in Advanced mode.',
             ...
             <div>
                ...
                ${dropdownButton(
                    activeResPreset.label,
                     activeResPreset.description || 'Custom restrictions applied.',
                    (e) => toggleDropdown(e.currentTarget, formattedOptionsDropdownTemplate(RESOLUTION_PRESETS, activeResPreset.id, (preset) => {
                         ...
                         playerService.setRestrictions({
                             minWidth: 0, minHeight: 0, minBandwidth: 0, maxBandwidth: Infinity,
                             ...preset.config
                         });
            ...
            <div>
                <label class="input-label mb-1">Buffering Profile</label>
                 ${dropdownButton(
                    ...
                    activeBufferPreset.description || 'Custom configuration applied.',
                    (e) => toggleDropdown(e.currentTarget, formattedOptionsDropdownTemplate(BUFFERING_PRESETS, activeBufferPreset.id, (preset) => {
                         ...
                         showToast({ message: `Buffering profile set to ${activeBufferPreset.label}`, type: 'pass' });
                    }))
                )}
            </div>
        </div>
...
const advancedModeTemplate = (config, handleFormChange, isAbrEnabled) => html`
    <div class="space-y-6">
        <div>
            <h5 class="text-xs font-bold text-gray-400 mb-2 uppercase">ABR Fine-Tuning</h5>
            <form data-form-id="abr-config" @change=${handleFormChange} class="space-y-3" ?disabled=${!isAbrEnabled}>
                 <div class="${!isAbrEnabled ? 'opacity-50' : ''}">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Higher = harder to upgrade.</p>
                </div>
                <div class="${!isAbrEnabled ? 'opacity-50' : ''}">
                    ...
                    <input type="number" step="0.05" min="0" max="2" name="bandwidthDowngradeTarget" id="bw-downgrade" class="input-field" .value=${config.abr.bandwidthDowngradeTarget}>
                     <p class="text-xs text-gray-500 mt-1">Lower = easier to downgrade.</p>
                </div>
            </form>
        </div>

        <div>
             <h5 class="text-xs font-bold text-gray-400 mb-2 uppercase">Explicit Restrictions</h5>
            <form data-form-id="abr-restrictions" @change=${handleFormChange} class="grid grid-cols-2 gap-3" ?disabled=${!isAbrEnabled}>
                <div>
        ...
        <div>
            <h5 class="text-xs font-bold text-gray-400 mb-2 uppercase">Buffering Engine</h5>
            <form data-form-id="buffering" @change=${handleFormChange} class="space-y-3">
                ...
                <div class="flex items-center gap-2 pt-1">
                    <input type="checkbox" name="ignoreTextStreamFailures" id="ignore-text-failures" .checked=${config.streaming.ignoreTextStreamFailures} class="rounded bg-gray-700 border-gray-600 text-blue-600 focus:ring-offset-gray-800">
...
export const playerControlsTemplate = () => {
    ...
    const videoButtonSubtext = isAbrEnabled ? `Up: ${config.abr.bandwidthUpgradeTarget}, Down: ${config.abr.bandwidthDowngradeTarget}` : (activeVideoTrack ? formatBitrate(videoBandwidthMap.get(activeVideoTrack.id) ?? activeVideoTrack.bandwidth) : 'N/A');
    ...
    // --- Section Content ---
    const masterControlsContent = html`
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center gap-2">
                ...
                <button @click=${() => reloadStream(stream)} class="px-4 py-2 font-bold rounded-md flex items-center gap-2 transition-colors bg-gray-600 hover:bg-gray-700 text-white" title="Reload manifest">
                    ${icons.updates}
                </button>
            </div>
    ...
    const trackSelectionContent = html`
        <div class="space-y-2 text-sm">
            ${dropdownButton(videoButtonLabel, videoButtonSubtext, (e) => toggleDropdown(e.currentTarget, videoSelectionPanelTemplate(videoTracks, isAbrEnabled, videoBandwidthMap)), !isAbrEnabled)}
            ${dropdownButton(audioButtonLabel, audioButtonSubtext, (e) => toggleDropdown(e.currentTarget, audioSelectionPanelTemplate(audioTracks)))}
            ${dropdownButton(textButtonLabel, textButtonSubtext, (e) => toggleDropdown(e.currentTarget, textSelectionPanelTemplate(textTracks)))}
        </div>
    `;
    ...
    return html`
        <style>
            .input-field { width: 100%; background-color: #1f2937; color: #f3f4f6; border-radius: 0.375rem; padding: 0.5rem 0.75rem; border: 1px solid #374151; transition: border-color 0.15s ease-in-out; }
            .input-field:focus { outline: none; border-color: #3b82f6; ring: 1px solid #3b82f6; }
            .input-label { font-size: 0.75rem; font-weight: 600; color: #9CA3AF; display: block; margin-bottom: 0.375rem; }
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 items-start">
                <div class="space-y-4 flex flex-col h-full">
                    ${controlSectionTemplate('Playback Control', masterControlsContent)}
                    <div class="grow">
                         ${controlSectionTemplate('Track Selection', trackSelectionContent)}
                    </div>
                </div>
                <div class="md:col-span-1 xl:col-span-2 h-full">
                     ${controlSectionTemplate(playerControlMode === 'standard' ? 'Playback Experience' : 'Advanced Configuration', configContent)}
                </div>
            </div>
        </div>
    `;
