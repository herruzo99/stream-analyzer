...
export const eventLogTemplate = (events) => {
    if (!events || events.length === 0) {
        return html`<div class="text-sm text-slate-500 italic">
    ...
    return html`
        <div
            class="bg-slate-900/50 rounded-lg border border-slate-700/50 h-full overflow-y-auto"
        >
                    ...
                    ${[...events].reverse().map(
                        (event) => html`
                            <tr class="hover:bg-slate-700/50">
                                <td
                                    class="p-2 font-mono text-slate-400 align-top"
                                >
                                    ${event.timestamp}
                                </td>
