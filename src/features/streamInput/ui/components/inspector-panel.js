...
const configSection = (title, icon, content) => html`
    <section class="mb-8 animate-fadeIn">
        <h3
            class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-slate-800 pb-2"
        >
...
const drmToggle = (systemName, detected, isConfigured, onClick) => html`
    <div
        @click=${onClick}
        class="flex items-center justify-between p-4 rounded-t-xl border-t border-x transition-all cursor-pointer select-none ${isConfigured
            ? 'bg-blue-900/10 border-blue-500/30'
            : 'bg-slate-900 border-slate-800 hover:bg-slate-800/80'} ${!isConfigured
            ? 'rounded-b-xl border-b'
            : ''}"
    >
        <div class="flex items-center gap-3">
            <div
                class="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-lg font-bold text-slate-400 border border-slate-700 shadow-inner"
            >
                ${systemName[0]}
            </div>
            <div>
        ...
        <div
            class="relative w-10 h-5 bg-slate-800 rounded-full border border-slate-700 transition-colors ${isConfigured
                ? 'bg-blue-600 border-blue-500'
                : ''}"
        >
            <div
                class="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${isConfigured
                    ? 'translate-x-5'
                    : 'translate-x-0'}"
            ></div>
        </div>
    ...
    const networkContent = html`
        <div class="bg-slate-800/30 rounded-xl p-4 border border-slate-800">
            <div class="flex justify-between items-center mb-4">
                ...
                <button
                    @click=${() =>
                        analysisActions.addAuthParam(activeInput.id, 'headers')}
            ...
            <div class="space-y-1">
                ${activeInput.auth.headers.length > 0
                    ? activeInput.auth.headers.map((h, i) =>
                          headerInputRow(h, i, activeInput.id, false)
                      )
                    : html`<div
                          class="text-center py-4 text-slate-600 text-xs italic border border-dashed border-slate-700 rounded-lg"
                      >
    ...
    const drmContent = html`
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ${['Widevine', 'PlayReady', 'FairPlay'].map((sys) => {
                ...
                const currentUrl =
                    typeof activeInput.drmAuth.licenseServerUrl === 'string'
                ...
                const certsObj =
                    ...
                    typeof activeInput.drmAuth.serverCertificate === 'object' &&
                    !(
                        activeInput.drmAuth.serverCertificate instanceof
                        ArrayBuffer
                    )
                        ? activeInput.drmAuth.serverCertificate
                ...
                const toggleConfig = () => {
                    const newUrlObj =
                        typeof activeInput.drmAuth.licenseServerUrl === 'object'
                            ? { ...activeInput.drmAuth.licenseServerUrl }
                            : {};
                    ...
                    analysisActions.updateStreamInput(
                        activeInput.id,
                        'drmAuth',
                        { ...activeInput.drmAuth, licenseServerUrl: newUrlObj }
                    );
                };
                ...
                const handleCertUrlInput = (e) => {
                    ...
                    analysisActions.updateStreamInput(
                        activeInput.id,
                        'drmAuth',
                        { ...activeInput.drmAuth, serverCertificate: newCerts }
                    );
                };
                ...
                const isCertFile =
                    currentCert && typeof currentCert === 'object';
                const isCertString = typeof currentCert === 'string';

                return html`
                    <div
                        class="rounded-xl shadow-sm transition-all duration-300 ${currentUrl
                            ? 'bg-slate-800/50 border border-blue-500/30'
                            : 'opacity-100'}"
                    >
                        ${drmToggle(
                            sys,
                            detectedDrm.includes(sys),
                            !!currentUrl,
                            toggleConfig
                        )}
                        ${currentUrl
                            ? html`
                                  <div
                                      class="p-4 border-x border-b border-blue-500/30 bg-slate-900/50 rounded-b-xl space-y-4 animate-fadeIn"
                                  >
                                      <!-- License URL -->
                                      <div>
                                          <label
                                              class="text-[10px] font-bold text-slate-500 mb-1.5 block uppercase tracking-wider"
                                      ...
                                      <div>
                                          ...
                                          ${isCertFile
                                              ? html`
                                                    <div
                                                        class="flex items-center justify-between bg-emerald-900/10 border border-emerald-500/30 rounded-lg p-2 animate-scaleIn"
                                                    >
                                                        <div
                                                        ...
                                                        >
                                                            ${icons.checkCircle}
                                                            <span
                                                                class="text-xs font-mono truncate max-w-[150px]"
                                                            >
                                                                ${currentCert.name ||
                                                                'Certificate Loaded'}
                                                            </span>
                                              ...
                                              : html`
                                                    ...
                                                    >
                                                        ...
                                                        />
                                                        <label
                                                            class="absolute inset-y-1 right-1 flex items-center cursor-pointer"
                                                        >
                                                            <input
                                                                type="file"
                                                                class="hidden"
                                                            ...
                                                            />
                                                            <div
                                                                class="p-1.5 rounded bg-slate-800 text-slate-400 hover:text-blue-400 hover:bg-slate-700 transition-colors border border-slate-700"
            ...
            <div class="bg-slate-800/30 rounded-xl p-4 border border-slate-800">
                ...
                <div class="space-y-1">
                    ${activeInput.drmAuth.headers.length > 0
                        ? activeInput.drmAuth.headers.map((h, i) =>
                              headerInputRow(h, i, activeInput.id, true)
                          )
                        : html`<div
                              class="text-center py-3 text-slate-600 text-xs italic"
                          >
                              No global license headers.
                          </div>`}
                </div>
            </div>
        </div>
