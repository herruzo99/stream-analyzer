...
const getBadge = (text, colorClasses) => html`
    <span
        class="text-[9px] font-black px-1.5 py-0.5 rounded border uppercase tracking-wider ${colorClasses}"
    >
        ${text}
    </span>
`;
...
const streamCard = (stream, context, onDelete) => {
    ...
    const isLive =
        stream.type === 'live' ||
        (stream.manifest && stream.manifest.type === 'dynamic');
    ...
    return html`
        ...
        >
            <div>
                <div class="flex justify-between items-start mb-3">
                    <div class="flex gap-2">
                        ...
                        ${isLive
                            ? html`<span
                                  class="text-[9px] font-bold text-red-400 bg-red-900/20 px-1.5 py-0.5 rounded border border-red-900/30 flex items-center gap-1"
                                  ><span
                              ...
                              >`
                            : html`<span
                                  class="text-[9px] font-bold text-slate-500 bg-slate-700/30 px-1.5 py-0.5 rounded border border-slate-700/50"
                                  >VOD</span
                              >`}
                    </div>
                    ${context !== 'examples'
                ...
                <h4
                    class="font-bold text-slate-200 text-sm mb-1 line-clamp-2 group-hover:text-white transition-colors"
                >
                    ${stream.name}
                </h4>

                <div
                    class="bg-black/20 rounded border border-white/5 p-1.5 mb-3 group-hover:border-white/10 transition-colors"
                >
                    <p
                        class="text-[10px] font-mono text-slate-500 truncate select-all"
                        title="${stream.url}"
                    >
                        ${stream.url}
                    </p>
            ...
            <div
                class="pt-3 border-t border-white/5 flex justify-between items-center mt-auto"
            >
                <div class="flex gap-2">
                    ${stream.auth?.headers?.length > 0
                        ? html`<span
                              class="text-xs text-slate-500"
                              title="Has Auth Headers"
                              >${icons.key}</span
                          >`
                        : ''}
                    ${stream.drmAuth?.licenseServerUrl
                        ? html`<span
                              class="text-xs text-slate-500"
                              title="DRM Configured"
                              >${icons.lockClosed}</span
                          >`
                        : ''}
                </div>
                <span
                    class="text-[10px] font-bold text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1"
                >
...
const workspaceCard = (workspace, onDelete) => {
    ...
    const handleLoad = () => {
        if (
            confirm(
                `Load workspace "${workspace.name}"? This will replace current streams.`
            )
        ) {
    ...
    return html`
        ...
        >
            <div class="flex justify-between items-start mb-4">
                <div
                    class="p-2.5 bg-teal-500/10 text-teal-400 rounded-lg border border-teal-500/20 group-hover:scale-110 transition-transform duration-300 shadow-inner"
                >
                    ${icons.folder}
                </div>
                <div
            ...
            <h4
                class="font-bold text-slate-200 text-base mb-1 group-hover:text-teal-300 transition-colors truncate"
            >
                ${workspace.name}
            </h4>
            ...
            <div class="space-y-1.5 mt-auto">
                ${workspace.inputs.slice(0, 3).map(
                    (input) => html`
                        <div
                            class="flex items-center gap-2 text-[10px] text-slate-400/80"
                        >
                            <div
                                class="w-1.5 h-1.5 rounded-full ${input.url.includes(
                                    '.m3u8'
                                )
                                    ? 'bg-purple-500/50'
                                    : 'bg-blue-500/50'}"
                            ...
                            <span class="truncate max-w-[200px]"
                                >${input.name ||
                                new URL(input.url).hostname}</span
                            >
                        </div>
                    `
                )}
                ${count > 3
                    ? html`<div
                          class="text-[9px] text-slate-600 pl-3.5 font-medium"
                      >
                          + ${count - 3} more
                      </div>`
                    : ''}
    ...
    render() {
        ...
        if (streamLibraryActiveTab === 'workspaces') {
                ...
                filtered.length > 0
                    ? html`<div
                      ...
                      >
                          ${filtered.map((w) =>
                              workspaceCard(w, () =>
                                  uiActions.deleteAndReloadWorkspace(w.name)
                              )
                          )}
                      </div>`
                    : html` <div
                          class="flex flex-col items-center justify-center h-full text-slate-500 border-2 border-dashed border-slate-800 rounded-2xl bg-slate-900/20 min-h-[200px]"
                      >
                          ...
                          <label
                              class="mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg cursor-pointer transition-colors shadow-lg shadow-blue-900/20"
                          >
                              Import JSON
                              <input
                                  type="file"
            ...
            const handleDelete = (url) => {
                if (context === 'history')
                    uiActions.deleteAndReloadHistoryItem(url);
            ...
            content =
                filtered.length > 0
                    ? html`<div
                          class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 auto-rows-max"
                      >
                          ${filtered.map((s) =>
                              streamCard(s, context, () => handleDelete(s.url))
                          )}
                      </div>`
                    : html` <div
                          class="flex flex-col items-center justify-center h-full text-slate-500 italic min-h-[200px]"
                      >
        ...
        const backdropTemplate = html`
            <div
                class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate-fadeIn"
            >
                <!-- Backdrop -->
                <div
                ...
                <div
                    class="relative w-full max-w-6xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl flex flex-col h-[85vh] animate-scaleIn overflow-hidden"
                >
                    <!-- Header / Toolbar -->
                    <div
                        class="shrink-0 p-5 border-b border-slate-800 bg-slate-900/80 backdrop-blur z-10 flex flex-col gap-4"
                    >
                        <div class="flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                <div
                                    class="p-2 bg-slate-800 rounded-lg border border-slate-700 text-slate-400"
                                >
                                    ${icons.inbox}
                                </div>
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500">
                                        Manage your saved environments and
                                        streams.
                                    </p>
                                </div>
                            </div>
                            <button
                                @click=${() => this.close()}
                                class="text-slate-500 hover:text-white transition-colors p-2 hover:bg-white/5 rounded-lg"
                            >
                                ${icons.xCircle}
                            </button>
                        </div>
                        ...
                        <div class="flex flex-col md:flex-row gap-4">
                            <!-- Tabs -->
                            <div
                                class="flex p-1 bg-slate-950 rounded-lg border border-slate-800 self-start overflow-x-auto max-w-full scrollbar-hide"
                            >
                                ${tabs.map((tab) => {
                                    const isActive =
                                        streamLibraryActiveTab === tab.key;
                                    return html`
                                        <button
                                            @click=${() =>
                                            ...
                                            class="px-4 py-2 rounded-md text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap ${isActive
                                                ? 'bg-slate-800 text-white shadow-sm ring-1 ring-white/10'
                                                : 'text-slate-500 hover:text-slate-300'}"
                                        >
                                            ${tab.icon} ${tab.label}
                                            <span
                                                class="opacity-50 text-[9px] bg-black/20 px-1.5 rounded-full"
                                                >${tab.count}</span
                                            >
                                        </button>
                                    `;
                            ...
                            <div class="relative grow">
                                ...
                                <input
                                    ...
                                    @input=${(e) =>
                                        uiActions.setStreamLibrarySearchTerm(
                                            e.target.value
                                        )}
                                />
                            </div>
                    ...
                    <div
                        class="grow overflow-y-auto custom-scrollbar p-6 bg-slate-950/30"
                    >
                        ${content}
                    </div>
                    ...
                    ${streamLibraryActiveTab === 'workspaces'
                        ? html`
                              <div
                                  class="shrink-0 p-4 bg-slate-900 border-t border-slate-800 flex justify-between items-center"
                              >
                                  <div class="text-[10px] text-slate-500">
                                      Workspaces persist your session queue,
                                      custom headers, and DRM settings.
                                  </div>
                                  <label
                                      class="cursor-pointer flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-lg text-xs font-bold text-slate-300 hover:text-white transition-colors"
                                  >
