...
function renderInputView(rerenderCallback) {
    const { streamInputActiveMobileTab } = useUiStore.getState();
    ...
    const template = html`
        <div class="w-full h-full max-w-screen-2xl mx-auto flex flex-col p-4 sm:p-6 lg:p-8">
            ...
            <div
                class="grow hidden lg:grid lg:grid-cols-[25%_45%_30%] xl:grid-cols-[25%_50%_25%] gap-6 min-h-0"
            >
                ...
                <div
                    class="flex flex-col min-h-0 bg-gray-800 rounded-lg border border-gray-700 animate-slideInUp"
                    style="animation-delay: 200ms;"
                >
                    ${inspectorPanelTemplate(rerenderCallback)}
                </div>
            </div>
            ...
            <div class=${classMap(mobileViewClasses)}>
                ...
                <div
                    class="fixed bottom-0 left-0 right-0 h-16 bg-gray-800/80 backdrop-blur-sm border-t border-gray-700 flex items-stretch z-10"
                >
...
export const inputViewTemplate = (rerenderCallback) => {
    return renderInputView(rerenderCallback);
};