...
const getTagHTML = (tagName, path, complianceResults) => {
    ...
    if (complianceResult) {
        ...
        )}" data-iso="${escapeHtml(complianceResult.isoRef)}"`;
    }
    const triggerClass =
    ...
    switch (node.nodeType) {
        case Node.ELEMENT_NODE: {
            ...
            const highlightClass = complianceResult
                ? `${
                      highlightColors[complianceResult.status]
            ...
            if (childNodes.length > 0) {
                ...
                const closingTag = `${openingTagStart}${indent}${getTagHTML(
                    `/${el.tagName}`,
                    path,
                    complianceResults
                )}&gt;</span></div>`;
                ...
                return [
                    `${openingTagStart}${indent}${getTagHTML(
                        el.tagName,
                        path,
                        complianceResults
                    )}${attrs} /&gt;</span></div>`,
