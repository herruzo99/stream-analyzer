...
const renderStreamListItem = (stream, isPreset) => {
    ...
    const handleSelect = () => {
        eventBus.dispatch('ui:stream-input:populate-from-preset', { id: activeStreamInputId, preset: stream });
        closeModal();
    };
    ...
    return html`
        <li class="group" @click=${handleSelect}>
            <div class="p-3 hover:bg-gray-700 cursor-pointer flex justify-between items-center">
                ...
                <div class="shrink-0 flex items-center gap-2 ml-4">
                    ...
                    ${isPreset || !isPreset ? html`
                        <button @click=${handleDelete} class="w-6 h-6 rounded-full flex items-center justify-center text-gray-500 hover:bg-red-800 hover:text-white opacity-0 group-hover:opacity-100 transition-opacity" title="Delete item">
...
const renderListSection = (title, items, isPreset) => {
    if (items.length === 0) {
        return html`<p class="text-center text-sm text-gray-500 p-8">No ${title.toLowerCase()} found.</p>`;
    }
    return html`
...
const renderExampleSection = () => {
    const filteredExamples = exampleStreams.filter(s => 
    ...
    return html`
        <div class="space-y-4">
            ${Object.entries(grouped).map(([groupName, items]) => html`
                <div>
                    <h4 class="font-bold text-gray-400 text-sm tracking-wider uppercase px-3 pt-3 pb-2 sticky top-0 bg-gray-800 z-10">${groupName}</h4>
                    <ul class="divide-y divide-gray-700/50">
...
export const streamLibraryModalTemplate = (rerender) => {
    ...
    if (activeTab === 'presets') {
        const presets = getPresets().filter(s => s.name.toLowerCase().includes(lowerSearch) || s.url.toLowerCase().includes(lowerSearch));
        content = renderListSection('Saved Presets', presets, true);
    } else if (activeTab === 'history') {
        const history = getHistory().filter(s => s.name.toLowerCase().includes(lowerSearch) || s.url.toLowerCase().includes(lowerSearch));
    ...
    const tabButton = (key, label) => html`
        <button @click=${() => { activeTab = key; rerender(); }} class="px-4 py-2 text-sm font-semibold rounded-t-lg transition-colors ${activeTab === key ? 'bg-gray-700 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700/50'}">
            ${label}
        </button>
    `;
    ...
    return html`
        <div class="flex flex-col h-full max-h-[80vh]">
            <div class="p-4 border-b border-gray-700 space-y-3 shrink-0">
                <input type="search" placeholder="Search streams..." .value=${searchTerm} @input=${e => { searchTerm = e.target.value; rerender(); }} class="w-full bg-gray-900 text-white rounded-md p-2 border border-gray-600 focus:ring-1 focus:ring-blue-500">
