...
function parseSpliceInfoSection(bs, rawData) {
    const section = {};
    ...
    // Calculate CRC (excluding the last 4 bytes which IS the CRC)
    // The section_length field starts 3 bytes into the data.
    // section_length includes the CRC. Total size = 3 + section_length.
    // We validate from start of table_id up to but not including CRC.
    const dataToValidate = rawData.subarray(0, 3 + section_length - 4);
    const calculatedCrc = calculateCRC32(dataToValidate);
    ...
    section.crc_valid = section.crc_32 === calculatedCrc;
...
export function parseScte35(data) {
    try {
        ...
        return parseSpliceInfoSection(bitstream, data);
    } catch (e) {
